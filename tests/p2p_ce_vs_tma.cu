...
// `tma_store` models the *fused* case: the tile was just produced in shared
// memory by a compute kernel and is pushed straight to the peer without a
// round trip through local global memory. It is an upper bound; its payload is
// intentionally not a faithful copy of the source, so verification is skipped.
//
...
struct Config {
  ...
  int dst_dev = 1;
  bool check = true;
  std::vector<std::string> methods{"ce", "sm", "tma", "tma_store"};
};
...
static void usage(const char* prog) {
  std::printf(
      ...
      "  --methods LIST    subset of ce,sm,tma,tma_store (default all)\n"
...
static TmaKernel pick_tma_kernel(int stages, bool store_only) {
  switch (stages) {
    case 3: return store_only ? tma_store_only_kernel<3> : tma_p2p_kernel<3>;
    case 4: return store_only ? tma_store_only_kernel<4> : tma_p2p_kernel<4>;
    case 5: return store_only ? tma_store_only_kernel<5> : tma_p2p_kernel<5>;
    case 6: return store_only ? tma_store_only_kernel<6> : tma_p2p_kernel<6>;
    case 8: return store_only ? tma_store_only_kernel<8> : tma_p2p_kernel<8>;
...
int main(int argc, char** argv) {
  ...
  for (uint64_t tokens : cfg.token_list) {
    ...
    if (wants(cfg, "ce")) {
      ...
      r.ok = time_transfer(launch, bytes, stream, cfg, "ce", &r);
      ...
      if (!can_peer) {
        ...
        int blocks = cfg.blocks > 0
                         ...
                         : (int)std::min<uint64_t>(
                               ...
                               (uint64_t)psrc.multiProcessorCount * 4);
        ...
        r.ok = time_transfer(launch, bytes, stream, cfg, "sm", &r);
        if (r.ok) r.verify = verify_dst();
        report("sm", r);
      }
    }
    ...
    for (int variant = 0; variant < 2; ++variant) {
      const bool store_only = (variant == 1);
      const char* name = store_only ? "tma_store" : "tma";
      ...
      TmaKernel k = pick_tma_kernel(ts.stages, store_only);
      if (!k) {
        Result r;
      ...
      r.ok = time_transfer(launch, bytes, stream, cfg, name, &r);
      if (r.ok) {
        if (store_only) {
          r.verify = -1;
          r.note = "payload not a faithful copy by design";
        } else {
          r.verify = verify_dst();
        }
      }
