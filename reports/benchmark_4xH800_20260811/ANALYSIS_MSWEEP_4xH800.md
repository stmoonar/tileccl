...
- **comm：2.19 (m=512) → 1.73 → 1.23 → 1.15 → 1.11 → 1.09 (16K) → ~~1.17~~ (32K)**。
  ...
  **【修订】32K 的 1.17 是本轮的测量假象**；复跑（300 iters）为 **1.099**，
  16K 同口径 1.105——平台一路延续到 32768，**曲线单调收敛后走平，没有回升**。
...
- **struct 非单调**：m≥4096 时 0.96–0.98，m=1024–2048 时 1.04–1.08（wave 太少，
  ...
  **【修订】"结构改造不亏反赚 4%" 撤回**：复跑同两个大 shape 正序只有 0.975–0.980、
  反序 0.980–0.991，0.96 没有复现，且其中约 +0.008 来自跑序偏置（先跑的变体吸收
  残余开销，而 base 恰恰排第一）。正确表述是**"结构改造免费，在跑序噪声内"**——
  m=1024–2048 的 1.04–1.08 那一段（wave 太少）不受影响，仍成立。
