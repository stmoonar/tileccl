...
- **【2026-08-12 追加，来自 m=32768 复跑与全 M-sweep 重标定】五条方法学教训，适用于本报告全部 fused 数字**：
  ...
  5. **warmup 数迭代次数是错的单位**：`--warmup 30` 在 M=32768 上是 0.68 s，
     在 M=512 上只有 0.022 s——小 shape 几乎等于没热身，计时窗口在稳态前就打开，
     使其 comm_sd 偏高最多 3%（fused 在 M=512/1024 上前后段漂移 −4.8% / −4.5%，
     M≥2048 归零）。已加 `--warmup-ms` 墙钟下限。
...
> **⚠ 2026-08-12 修订**：整条曲线已按 300 iters / 30 warmup 在同机同频下重标定
> （原为 50 / 5）。**除 m=32768 外逐点复现，偏差 ±2.3%**；只有 m=32768 偏了
> −5.9%，是唯一落在轮间波动之外的点，其 1.17 / 1.13 系测量假象。
>
> | M | 512 | 1024 | 2048 | 4096 | 8192 | 16384 | 32768 |
> |---|---|---|---|---|---|---|---|
> | comm 重标定 | 2.235 | 1.691 | 1.246 | 1.157 | 1.101 | 1.103 | **1.101** |
> | struct 重标定 | 1.008 | 1.043 | 1.052 | 0.997 | 1.021 | 0.977 | 0.977 |
> | total 重标定 | 2.243 | 1.774 | 1.311 | 1.166 | 1.110 | 1.074 | **1.074** |
>
> **comm 平台延续到 32768，M 方向无上界**（每 tile 残差在 M≥8192 走平于
> 85/86/84 ns，故平台是结构性的）。**struct 全程 0.98–1.05，本表的 0.96 未复现**。
> 小 M 点因 warmup 按迭代数计而偏高 ≤3%（详见复跑分析 §6）。
> 完整数据与分析：`../benchmark_4xH800_20260812/ANALYSIS_RERUN_M32768.md`。

**读法**
- **结构改造全程免费**：struct_sd 在所有 M、K 上都是 0.96–1.08——把 2 个空转 warp（CLC 调度/MainloopAux）改成 RS Fetch/Reduce、加 swizzle、smem carveout，本身不亏不赚。~~（大 M 下因 stage 少一档略快 0.96）~~ **【2026-08-12 修订】括号内的"倒赚 4%"撤回**：重标定后整条 struct 曲线为 1.008/1.043/1.052/0.997/1.021/0.977/0.977，**全程 0.98–1.05，无一点接近 0.96**；且 struct_sd 本身带跑序偏置（本实验固定 base 先跑，先跑者吸收残余开销，反序后平均 +0.008）。结论收敛为"免费"，不存在增益。
- **全部代价来自通信**（comm_sd）。M-sweep 给出清晰的「随 M 增长通信被掩藏」曲线：
  ...
  - ~~**M=32768 comm_sd 回升到 1.17**——平台并非无限延续，尾部方差开始抬头。total_sd 最小点在 M=16384（1.05）。~~ **【2026-08-12 已推翻】** 复跑得 comm 1.101（全 M-sweep 重标定，p50），同口径 M=16384 为 1.103、M=8192 为 1.101，**三点打平，平台延续到 32768 且 M 方向无上界**；超 p50 5% 的迭代占比 0%，肥尾消失。绝对残差 C=fused−ctrl 从本轮的 74→137 ns/tile（超线性，当初的报警依据）变为重标定后的 85/86/84 ns/tile（M≥8192 完全走平），与 ctrl 同比例增长，所以走平是结构性的。成因未定：短 warmup 已排除，剩下"长套件末端热累积"（本轮 m=32768 是 40 分钟套件的最后一点），未证实。
